...
def return_answer(request, client_socket):
    ...
    if request == 'DIR':
        try:
            folder_name_bytes = Protocol.receive(client_socket)
            ...
            Protocol.send(client_socket, output)
            return x
        ...
        try:
            ...
            file_name = Protocol.receive(client_socket)
            ...
            Protocol.send(client_socket, b"File deleted successfully.")
        except socket.error as err:
            logging.debug('file not found')
    elif request == 'COPY':
        try:
            source = Protocol.receive(client_socket)
            print('source: ' + source)
            destination = Protocol.receive(client_socket)
        ...
        try:
            program_path = Protocol.receive(client_socket)
            print('program_path: ' + program_path)
            subprocess.call(program_path)
            Protocol.send(client_socket, 'executed')
        except socket.error as err:
        ...
        while True:
            ...
            try:
                while response != 'exit':
                    ...
                    request = Protocol.receive(client_socket)
                    print('request: ' + request)
